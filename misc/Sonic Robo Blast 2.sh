#!/bin/bash
GAMEDIR="$( cd "$( dirname "${BASH_SOURCE[0]}" )" >/dev/null 2>&1 && pwd )/Sonic Robo Blast 2"
...
if id "ark" &>/dev/null || id "odroid" &>/dev/null; then
  sudo $BINDIR/oga_controls srb2.elf &
else
  $BINDIR/oga_controls srb2.elf &
fi

./srb2.elf 2>&1 | tee -a ./log.txt
